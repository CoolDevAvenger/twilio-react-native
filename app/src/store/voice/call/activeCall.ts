...
import { callMap, voice } from '../../../util/voice';
...
>(sendDigitsActionType.prefix, async ({ id, digits }, { rejectWithValue }) => {
  ...
  // const twiml = `
  //   <Response>
  //     <Say>This is additional information for the call.</Say>
  //   </Response>
  // `;

  // // Use the Voice SDK to send the TwiML to Twilio's servers for an ongoing call
  // await Voice.sendDigits(callSid, {
  //   twiml: twiml,
  // });

  const sendDigitsResult = await settlePromise(call.sendDigits(digits));
  console.log("sendDigitsActiveCall", digits, call, sendDigitsResult);
